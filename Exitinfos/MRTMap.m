...
- (id)initWithFrame:(CGRect)frame AndSuperView:(UIView *)theView
{
    ...
    if (self) {
        if ([[[NSUserDefaults standardUserDefaults] stringForKey:@"BackgroundColor"] isEqualToString:@"WhiteBackground"])
            ...
            self.backgroundColor = [theColor ClassicGrayBackground];
        ...
        // All about gesture recognizer
        UILongPressGestureRecognizer *longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(dismissFunctionViewByGesture)];
...
-(void)DrawLine:(int)Type Color:(NSString *)ColorName Start:(CGPoint)StartCoordinate Destiny:(CGPoint)DestinyCoordinate
{
    ...
    switch (Type) {
        ...
        case 15:
            ...
            CGContextSetLineWidth(context, 9);
            CGContextBeginPath(context);
            CGContextMoveToPoint(context, (DestinyCoordinate.x+(3*sqrt(2))+StartCoordinate.x+(3*sqrt(2)))/2, (DestinyCoordinate.y-(3*sqrt(2))+StartCoordinate.y-(3*sqrt(2)))/2);
            CGContextAddLineToPoint(context, DestinyCoordinate.x+(3*sqrt(2)), DestinyCoordinate.y-(3*sqrt(2)));
            CGContextClosePath(context);
            CGContextSetStrokeColorWithColor(context, [self RGBColor:@"dark green"]);
            [[UIColor grayColor] setFill];
            CGContextDrawPath(context, kCGPathFillStroke);
            CGContextSetLineWidth(context, 9);
            CGContextBeginPath(context);
            ...
            CGContextClosePath(context);
            CGContextSetStrokeColorWithColor(context, [self RGBColor:@"green"]);
            [[UIColor grayColor] setFill];
            CGContextDrawPath(context, kCGPathFillStroke);
            break;
        case 16:
...
-(void)DrawCircle:(int)Type Color:(NSString *)ColorName Coordinate:(CGPoint)StationCoordinate
{
    ...
    switch (Type) {
        ...
        case 7:
            ...
            CGContextClosePath(context);
            CGContextSetFillColorWithColor(context, [self RGBColor:@"green"]);
            CGContextDrawPath(context, kCGPathFillStroke);
            break;
        case 8:
            CGContextSetLineWidth(context, 0);
        ...
        case 9:
            ...
            CGContextSetLineWidth(context, 0);
            CGContextBeginPath(context);
            CGContextAddArc(context, StationCoordinate.x+10/sqrt(2), StationCoordinate.y-10/sqrt(2), 5, -3*M_PI/4, -7*M_PI/4, 0);
            CGContextAddArc(context, StationCoordinate.x-10/sqrt(2), StationCoordinate.y+10/sqrt(2), 5, -7*M_PI/4, -3*M_PI/4, 0);
            CGContextClosePath(context);
            [[UIColor blackColor] setFill];
            CGContextDrawPath(context, kCGPathFillStroke);
            break;
...
- (void)dismissFunctionViewByGesture {
    for (int i = 0; i < [AllStationCoordinate count]; i++) {
        if (i == [AllStationCoordinate count]-1) {
            [DismissFunctionDelegate DismissFunctionView:self];
        }
