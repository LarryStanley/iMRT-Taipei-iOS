...
@synthesize PriceAndTimeIsShow,FunctionViewShowMoreDelegate,ChangeStatusBarColorDelegate,ExitInfosView,SetExitDelegate,StartStationName,ArrowImage,SetTransferDelegate,TransferView,IllustratorGrayView;
- (id)initWithFrame:(CGRect)frame StationName:(NSString *)Name StationColor:(NSString *)Color AndStationNumber:(int)Number
{
    ...
    if (self) {
        ...
        //SwipeGestureRecognizerUp = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(ShowHigherView)];
        //SwipeGestureRecognizerUp.direction = UISwipeGestureRecognizerDirectionUp;
        //SwipeGestureRecognizerDown = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(DismissHigherView)];
...
-(void)ShowInMapButtonPressed
{
    if (FunctionViewIndex != 2) {
        ...
        if (!HigherViewIsShow) {
            ...
            if (self.superview.frame.size.height == 460)
                [ShowInMapScrollView setContentOffset:CGPointMake(0, 35) animated:YES];
            [self ShowHigherView];
            [self setNeedsDisplay];
        }else{
