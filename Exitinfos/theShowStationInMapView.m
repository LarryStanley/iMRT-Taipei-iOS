...
-(void)ShowMap
{
    ...
    //設定地圖位置等設定
    if (self.superview.frame.size.height < 500){
        StationMap = [[MKMapView alloc] initWithFrame:CGRectMake(20, 20, 280, 230)];
        ShowInMapButton = [[theGrayButton alloc] initWithFrame:CGRectMake(self.bounds.size.width/2-118, 270, 108, 33) AndButtonText:NSLocalizedString(@"ShowInMap", nil)];
        RouteNavigateButton = [[theGrayButton alloc] initWithFrame:CGRectMake(self.bounds.size.width/2+10, 270, 108, 33) AndButtonText:NSLocalizedString(@"Route", nil)];
    }else{
